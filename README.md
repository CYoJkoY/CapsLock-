...
By holding `CapsLock` and combining it with other keys, you can perform Vim‑style home row cursor movement, advanced clipboard management (including encrypted history), window transparency adjustment, tab switching, AI‑powered text processing, document format conversion via Pandoc, cloud config sync via GitHub Gist, and many other workflow automations—all without leaving the keyboard's home row.
...
<div style= "background: #2A2A2A; border-radius: 20px; padding: 16px; margin: 16px 0; " >
 <h3 style= "margin-top: 0; color: #D6D2CC; " >☁️ Gist Cloud Sync </h3 >
 <ul style= "color: #BEB8AE; " >
 <li >Sync your <code >configs/</code > folder (Config.ini + .prompt files) to a private GitHub Gist </li >
 <li >Upload, append (skip existing), or overwrite (full replace) with one click </li >
 <li >Accessible from tray menu → ☁️ Gist Sync </li >
 <li >Non‑UTF‑8 files are automatically Base64‑encoded for safe transfer </li >
 </ul >
 </div >
...
- **ImageMagick** (optional) – Required for image‑to‑PDF feature; install from [imagemagick.org](https://imagemagick.org/) (check "Install legacy utilities" during setup)
- **Pandoc** (optional) – Required for document conversion feature; install from [pandoc.org](https://pandoc.org/)
...
1. Right‑click the tray icon → click "☁️ Gist Sync".
2. Enter your GitHub Personal Access Token (requires `gist` scope).
...
| Gist Sync|Open the GitHub Gist cloud sync GUI|
...
ActivePrompt=Translate to Chinese
...
Prompts are stored as individual `.prompt` files in `configs/prompts/`. Each file's name (without extension) is the prompt name, and its content is the prompt text. The active prompt name is tracked in `Config.ini` under `[AI] ActivePrompt`.
...
│   └── 🖼️ Utils.ico
├── 📁 Config
│   ├── 📄 ConfigManager.ahk     ; INI config load/save (includes AI & Gist settings)
│   ├── 📄 Encryption.ahk        ; XOR encryption for history & config storage
│   ├── 📄 Globals.ahk           ; AppState class — all constants & settings
│   ├── 📄 PromptManager.ahk     ; Prompt file-based storage (add/edit/delete/active)
│   └── 📄 SecureIni.ahk         ; Encrypted INI read/write (XOR stream cipher)
├── 📁 Core
...
│   ├── 📄 GistSync.ahk          ; GitHub Gist upload/download sync engine
...
├── 📁 UI
│   ├── 📄 GistSyncGui.ahk       ; Gist cloud sync settings GUI
...
│   ├── 📄 PandocUtils.ahk       ; Pandoc format detection & path helpers
│   ├── 📄 ResourceSound.ahk     ; Embedded WAV resource playback
│   └── 📄 WindowUtils.ahk       ; Window opacity adjustment
...
- **Encrypted config file** – `Config.ini` is stored as a binary file with XOR stream encryption (magic header `0x31585C43`), preventing casual inspection of API keys and tokens.
...
- **AI API integration** – Full OpenAI‑compatible chat completion support with streaming, prompt management via `.prompt` files, and a dedicated result window with copy/paste shortcuts.
- **File‑based prompt persistence** – Each prompt is stored as a `.prompt` file in `configs/prompts/`; the active prompt name is tracked in `Config.ini`. All operations (add/edit/delete/activate) are reflected immediately on disk.
...
> ⚠️ The config file `configs/Config.ini` is encrypted with the same XOR stream cipher. API keys and Gist tokens are obfuscated but not cryptographically secure.

If you handle highly sensitive data, it is recommended to:
- Set `ENCRYPT_KEY` to `0` (disable encryption) in `Config/Globals.ahk`
