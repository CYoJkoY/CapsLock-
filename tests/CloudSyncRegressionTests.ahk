...
RunTests() {
    ...
    cloudSyncGuiSource := FileRead(
        A_WorkingDir "\UI\CloudSyncGui.ahk",
        "UTF-8"
    )

    Assert(
        InStr(cloudSyncGuiSource, 'OnEvent("Destroy"') == 0,
        "Cloud Sync GUI must not register the invalid Destroy GUI event."
    )
    Assert(
        InStr(
            cloudSyncGuiSource,
        ...
        "Cloud Sync GUI close paths are not centralized."
    )
