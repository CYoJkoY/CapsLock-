...
RunTests() {
    ...
    hotkeyReference := FileRead(root "\Hotkeys\HotkeyReference.ahk", "UTF-8")
    ...
    Assert(!InStr(windowHole, "FallbackMinimized"), "Obsolete FallbackMinimized state map is still present.")

    Assert(InStr(windowHole, "static _GetPhysicalCursorPosition(&x, &y)") > 0, "Window Hole has no DPI-normalized cursor coordinate helper.")
    Assert(InStr(windowHole, "static _GetPhysicalWindowGeometry(hwnd") > 0, "Window Hole has no DPI-normalized window geometry helper.")
    Assert(InStr(windowHole, "DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2 := -4") > 0, "Window Hole is missing the Per-Monitor V2 DPI context.")
    Assert(InStr(windowHole, "SetThreadDpiAwarenessContext") > 0, "Window Hole does not normalize the thread DPI context.")
    Assert(InStr(windowHole, "GetWindowRect") > 0, "Window Hole does not read window geometry directly.")

    Assert(InStr(windowHole, "DWM_NCRENDERING_POLICY := 2") > 0, "Window Hole is missing DWM non-client rendering policy support.")
    ...
    Assert(InStr(hotkeyReference, 'keys: "CapsLock + X"') > 0, "Window Hole shortcut is missing from the hotkey reference.")
    Assert(InStr(hotkeyReference, 'keys: "CapsLock + X, then 1"') > 0, "Window Hole second-level shortcut is missing from the hotkey reference.")
    Assert(InStr(hotkeyReference, 'keys: "CapsLock + H / F1"') > 0, "Hotkey reference does not document both cheatsheet shortcuts.")
    Assert(InStr(lang, "CHEAT_ACT_QUICK_PHRASE") > 0, "Missing Quick Phrase cheatsheet translation.")
    Assert(InStr(lang, "CHEAT_ACT_WINDOW_HOLE") > 0, "Missing Window Hole cheatsheet translation.")
    Assert(InStr(lang, "CHEAT_ACT_WINDOW_HOLE_SECOND") > 0, "Missing Window Hole second-level translation.")
