...
RunTests() {
    ...
    Assert(
        InStr(quickPhraseSource, "QuickPhraseTransactionActive") > 0
            && InStr(quickPhraseSource, "QuickPhraseDestroySelector(selectorGui)") > 0
            && InStr(quickPhraseSource, "QuickPhraseExecutePhrase") > 0
    ...
    Assert(
        ...
        "Quick Phrase must not depend on a stale captured child control HWND."
    )
    Assert(
        InStr(quickPhraseSource, "WinActivate(\"ahk_id \" targetHwnd)") > 0
            && InStr(quickPhraseSource, 'Send("^v")') > 0,
        "Quick Phrase final insertion must restore the original target window."
    )
    Assert(
        InStr(quickPhraseSource, "Multi WantReturn") > 0
            && InStr(quickPhraseSource, "etxtControl") > 0,
        "Quick Phrase variable input is not multiline."
    )
