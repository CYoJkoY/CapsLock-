...
class PromptEditDialog {
    ...
    static Show(title, existingName := "", existingContent := "") {
        ...
        nameEdit := myGui.Add(
            "Edit",
            "w480 r1 y+6 " . ThemeHelper.GetEditOptions(),
            existingName
        )
        nameEdit.Focus()
        ...
        contentEdit := myGui.Add(
            "Edit",
            "Multi VScroll w480 h200 y+6 " . ThemeHelper.GetEditOptions(),
            existingContent
        )
        myGui.SetFont("s10 c" AppState.THEME_FG, AppState.THEME_FONT)

        ; Buttons (wider to prevent text wrapping)
        btnOK := ThemeHelper.AddButton(myGui, "Default w100 y+18", "✓ " . Lang("GUI_OK"), "primary")
        btnCancel := ThemeHelper.AddButton(myGui, "x+10 w100", "✕ " . Lang("GUI_CANCEL"))
